...
public class Runtime {
     ...
     */
    private enum States {notStarted, starting, started, shuttingDown, shutDown};
     ...
     * singleton pattern in state "notStarted".
     * @return Runtime singleton instance (notStarted)
     */
    ...
    public <T> Optional<T> getBean(Class<T> clazz) {
        // make sure Runtime is started when called from tests
        return (clazz==null)? Optional.empty() : start().runnables.stream()
     ...
     * but performs actions only once at the first invocation.
     * @return chainable self-reference
     */
    public Runtime start() {
        return state==States.started? this : start(new String[] { });
    }
